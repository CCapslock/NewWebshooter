...
public class JokerView : MonoBehaviour, IBoss
{
    ...
    public void ChangeState(JokerStates state)
    {
        ...
        switch (_state)
        {
            case JokerStates.Waiting:
                {
                    Debug.LogWarning("Включить Тут Партикл Смехуечков");
                    //включить партикл смехуечков
                }
                break;
    ...
    public void SpawnEnemy()
    {
        ...
        var obj = Instantiate(_jokerMinion, _rightHand.transform.position, Quaternion.identity);
