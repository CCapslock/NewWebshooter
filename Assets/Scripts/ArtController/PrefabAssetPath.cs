using System.Collections.Generic;

public class PrefabAssetPath
{
	public static readonly Dictionary<string, string> LevelParts = new Dictionary<string, string>
	{
		{"EnemyPrefabWithRagdoll", "Prefabs/EnemyPrefabWithRagdoll"},
		{"EnemyThrowingBombs", "Prefabs/EnemyThrowingBombs"},
		{"EnemyFalling", "Prefabs/EnemyFalling"},
		{"EnemyPrefabWithShield", "Prefabs/EnemyPrefabWithShield"},
		{"EnemyPrefabDodge", "Prefabs/EnemyPrefabDodge"},
		{"PrepearedBuilding1", "Prefabs/PrepearedBuilding1"},
		{"PrepearedBuilding2", "Prefabs/PrepearedBuilding2"},
		{"PrepearedBuilding3", "Prefabs/PrepearedBuilding3"},
		{"PrepearedBuilding4", "Prefabs/PrepearedBuilding4"},
		{"PrepearedBuildingConstr1", "Prefabs/PrepearedBuildingConstr1"},
		{"PrepearedBuildingConstr2", "Prefabs/PrepearedBuildingConstr2"},
		{"PrepearedRoofBuilding", "Prefabs/PrepearedRoofBuilding"},
		{"PrepearedDecorativeBench", "Prefabs/PrepearedDecorativeBench"},
		{"PrepearedDecorativeCrane", "Prefabs/PrepearedDecorativeCrane"},
		{"PrepearedDecorativeVoltageWires", "Prefabs/PrepearedDecorativeVoltageWires"},
		{"PrepearedDecorativeWoodShield", "Prefabs/PrepearedDecorativeWoodShield"},
		{"BankBuilding", "Prefabs/BankBuilding"},
		{"WebSphere", "Prefabs/WebSphere"},
		{"WebModel", "Prefabs/WebModel"},
		{"MovingPoint", "Prefabs/MovingPoint"},
		{"BuildingCube", "Prefabs/BuildingCube"},
		{"Map", "Maps/Map"},
		{"BossSet0", "Prefabs/BossSet0"},
		{"BossSet1", "Prefabs/BossSet1"},
		{"BossSet2", "Prefabs/BossSet2"},
	};
}
